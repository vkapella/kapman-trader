...
OUTPUT_DIR = Path(__file__).resolve().parents[2] / "outputs" / "exp_spring_to_sos_entry_v1"
...
def main() -> None:
    ...
    logger.info("Applying SPRING -> SOS experiment filter.")
    ...
    if filtered.empty:
        logger.warning("No events produced for %s; skipping benchmark.", experiment_id)
        return

    filtered.to_parquet(events_out_parquet, index=False)
    filtered.sort_values(["symbol" if "symbol" in filtered.columns else "ticker", "event_date"]).to_csv(
        events_out_csv, index=False
    )
