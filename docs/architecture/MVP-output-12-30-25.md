...
## 0. Benchmark vs. Production

Note on Benchmark vs Production Output Alignment

This benchmark output represents a research-grade Wyckoff event classification used to validate detection logic, sensitivity, and coverage across a broad symbol universe. Its primary purpose is exploration and validation, not direct production parity.

The production Wyckoff algorithm is intentionally designed to produce tighter, more conservative results and therefore will not exactly match this benchmark. Controlled divergence is expected and correct.

Key Reasons for Expected Drift
	1.	Event Precedence and Suppression
In production, Wyckoff events are not treated as independent. Later-stage or regime-defining events (e.g., BC, SOW) explicitly dominate and suppress weaker or local events (e.g., AR, SOS, SPRING).
The benchmark allows more concurrent and marginal event emissions for research visibility.
	2.	Confirmation and Path Dependence
The production engine enforces contextual and temporal confirmation for certain events, favoring regime stability over single-bar detection.
The benchmark may emit events on the same bar conditions are first met.
	3.	Noise Reduction and Threshold Hardening
Production applies stricter thresholds, normalization, and minimum-history requirements to reduce false positives and event churn.
The benchmark is intentionally more permissive to surface edge cases during research.
	4.	Operational Intent
Benchmark output maximizes recall and diagnostic insight.
Production output maximizes determinism, interpretability, and downstream decision quality.

Interpretation Guidance
	•	Exact parity between benchmark and production is not a success criterion.
	•	Differences clustered around known Wyckoff boundaries (e.g., SOS ↔ UT, SPRING ↔ SOW/BC, AR ↔ BC) are expected.
	•	The benchmark should be treated as a historical research reference, while the production algorithm is the authoritative operational implementation.

This distinction is intentional and reflects the documented evolution from research validation to production-grade structural analysis.
