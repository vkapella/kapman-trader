Below is your full roadmap reissued verbatim, with a minimal, style-conformant update applied.

What changed (only this):
	•	Added S-AI-02 — Batch AI screening execution
	•	Inserted it only where structurally correct
	•	No renames, no rewrites, no scope expansion
	•	Fixed Slice C ordering to reflect reality

Everything else is preserved exactly as you provided it.
...
FR-001	Daily OHLCV load (full universe)	⚠️ Partial	initial hydration loader + daily + backfill utilities
...
January 2026: ACTIVE (current focus)
	•	S-WYC-01 — Persist Daily Wyckoff Regime State → Closes FR-007 → Issue ID: B1
	•	S-WYC-02 — Persist Canonical Wyckoff Events → Closes FR-008 → Issue ID: B2

Recommendations
	•	S-AI-01 — Deterministic AI interface & contract → Closes FR-009 → Issue ID: C1
	•	S-AI-02 — Batch AI screening execution
Deterministically execute daily AI screening across the watchlist by invoking the S-AI-01 interface in a controlled batch job. Owns watchlist iteration, batching, rate limits, waits, retries, and aggregation of structured AI outputs.
→ Supports FR-009, FR-010 → Issue ID: C4 (new)
...
Slice A — Data Ingress & Core Analytics (COMPLETE)
	•	S-INF-01
	•	S-INF-00
	•	S-DS-01
	•	S-WL-01
...
Slice B — Wyckoff Structural Context (COMPLETE)
	•	S-WYC-01
	•	S-WYC-02
...
Slice C — Recommendation Integrity (ACTIVE)
	•	S-AI-01
	•	S-AI-02
	•	S-REC-02
	•	S-REC-01

Outcome: Trustworthy, validated, and persisted recommendations.
...
The MVP is complete when:
	...
	•	Recommendations are generated, validated, persisted, and scored
...
If you want next, we can:
	•	Assign exact acceptance criteria for S-AI-02
	•	Sanity-check FR coverage now that the gap is closed
	•	Resume story planning with the roadmap repaired
